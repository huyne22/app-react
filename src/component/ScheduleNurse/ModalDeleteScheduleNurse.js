...
import { postDeleteScheduleNurse } from "../service/apiService";
const ModalDeleteScheduleNurse = (props) => {
  const { dataUpdate, fetchListSchedule, show2, setShow2 } = props;
  const handleShow2 = () => setShow2(true);
  const handleClose2 = () => setShow2(false);
  const handleSubmit2 = async (e) => {
    let a = { Ngay: dataUpdate.Ngay.toString() };
    let res = await postDeleteScheduleNurse(
      dataUpdate.Ngay,
      dataUpdate.Buoi,
      dataUpdate.MaYTa
    );
...
export default ModalDeleteScheduleNurse;
